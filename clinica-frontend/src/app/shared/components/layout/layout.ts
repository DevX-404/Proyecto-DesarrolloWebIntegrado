...
export class LayoutComponent implements OnInit {
  ...
  obtenerDatosDeSesion(): void {
    // 1. Primero intentamos jalar el nombre directamente si se guardó en el login
    const usuarioGuardado = localStorage.getItem('username');
    if (usuarioGuardado) {
      this.username = usuarioGuardado;
    }

    // 2. Si no, lo extraemos decodificando el sub del Token JWT por seguridad
    ...
    if (token) {
      try {
        ...
        if (payload.sub) {
          this.username = payload.sub; // Guarda "admin" o "medico"
        }
      } catch (e) {
  ...
  configurarMenuPorNombreUsuario(): void {
    // Definición de iconos SVG limpios
    ...
    // Convertimos a minúsculas para evitar fallos por espacios o caracteres
    const usuarioLimpio = this.username.toLowerCase().trim();

    // 🟢 SEGMENTACIÓN DE VISTAS POR NOMBRE DE CUENTA EN TIEMPO REAL
    if (usuarioLimpio === 'medico') {
      // Menú exclusivo para el perfil del Doctor Mendoza
      this.menuItems = [
      ...
      // Menú completo para la cuenta 'admin' y cualquier otro gestor
      this.menuItems = [
        { label: 'Dashboard', route: '/dashboard', icon: iconDashboard },
